...
raw_dataset = 'data/trec/{:s}.npy'
class_range = ['ABBR', 'DESC', 'ENTY', 'HUM', 'LOC', 'NUM']
...
def extract_inst_label_pair(file, pack=False, ignore_label=False):
    ...
    for item in file:
        ...
        if not ignore_label:
            label = class_range.index(item['label'])
            labels.append(label)
        else:
            pack = False
    if pack:
...
def build_allentrainer(model, train_dl, val_dl, save_loc='output/'):
    ...
    optimizer = AdamWOptimizer(parameters, lr=3e-5)
    default_epoch = 25
...
def main(soft_label_loc=None, rand=-2):
    ...
    model = BertClassifier(
        vocabulary, bert_textfield_embedder, freeze_encoder=False, out_features=6)
